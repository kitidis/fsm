    ...
    while len(states) > 0:
        ...
        for state in states:
            for t_id in fsm['transitions']:
                ...
                if state != int(transition['s1']):
                    continue

                s2 = int(transition['s2'])
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("Tool for check connectivity of FSM v 1.0")
    ...
    ret, errors = is_connected(sys.argv[1])
    if ret:
        print('FSM connected')
    else:
        print('FSM not connected')
        print('Not connected state IDs: ' +errors)